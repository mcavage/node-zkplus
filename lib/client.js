...
function onZookeeperError(zh, p, code) {
    ...
    switch (code) {
        ...
        case ZK.ZBADARGUMENTS:
            this.emit('error', new ZKError(code, 'ZK: invalid arguments'));
        break;
    ...
    zk.a_exists(_p, false, function existsCallback(rc, msg) {
        ...
        function onChildren(rc2, msg2, children) {
            ...
            if (!firstFire && rc2 === ZK.ZNONODE) {
                ...
                /*
                 * #34
                 * We don't emit the error if the watch is
                 * done. This is for 2 reasons:
                 * 1) The watch has been marked done which
                 * means we are no longer interested.
                 * 2) When the watch is done, we can't be sure
                 * that the error listeners still exist,
                 * emitting error in this case could cause
                 * consumers to abort if there are no 'error'
                 * listeners.
                 */
                log.trace({ path: _p, rc: rc2, message: msg2 },
        ...
        function onData(rc2, msg2, _stat, data) {
            log.trace({ rc: rc2, msg: msg2, stat: _stat, data: data },
                      'watch: onData');
            // delete event if znode DNE when we reset the watch
            if (!firstFire && rc2 === ZK.ZNONODE) {
                log.trace({ path: _p, err: err },
                          'watch: znode dne, emitting delete event');
                return (emitter.emit('delete'));
            } else if (rc2 !== 0 && !done) { // #34
                var err = new ZKError(rc2, msg2);
                log.trace({ path: _p, err: err }, 'watch: error getting data');
            ...
            try {
                ...
                // #34
                if (!done) {
                    log.error({err: e}, 'error while parsing data', e);
                    return (self.emit('error', e));
                }
            }

            log.trace({ path: _p, obj: obj, stat: _stat },
                      'watch: data received');
