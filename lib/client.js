...
function ZKClient(options) {
        ...
        this.zk.on('session_expired', function onSessionExpired() {
                self.zk.removeAllListeners('error');
                self.zk.on('error', function (z, p, c) {
                        if (c !== ZK.ZINVALIDSTATE) {
                                onZookeeperError(this, z, p, c);
                                return;
                        }
                        // otherwise just eat this...
                });
