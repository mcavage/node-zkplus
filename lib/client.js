...
ZKClient.prototype.watchPathExistence = function wpe(p, callback) {
        var parents;
        var recurse = arguments[2];
        var run = arguments[3];
        ...
        if (parents.length > 1) {
                ...
                // prepare recurse function if needed
                var dorecurse = zk.watchPathExistence.bind(
                        zk,
                        parents.slice(1),
                        callback,
                        true,
                        run);
                var params = {method: 'list', initialData: true};
                zk.watch(cur, params, watchcb);
        }

        function watchcb(err, listener) {
                if (err !== null) {
                        return (callback(err));
                }

                listener.on('error', errListener);
                listener.on('session', sessionListener);
                return (listener.on('children', childrenListener));
        }
        ...
        function sessionListener() {
                // as a work around on the session event bug,
                // we unregister all when it happens and stop
                this.removeAllListeners();
                this.stop();
                return (callback(new Error('Session event fired')));
        }

        function childrenListener(children) {
                if (!run) {
                        return (this.stop());
                }
                if (children.indexOf(path.basename(next)) !== -1) {
                        if (next === last) {
...
ZKClient.prototype.uwatch = function uwatch(p, callback) {
        assert.string(p, 'path');
        assert.func(callback, 'callback');

        var zk = this;
        var states = {START: 0, NO_NODE: 1, BAD_DATA: 2, OK: 3};
        var state = states.START;
        var run = true;

        var pathrun = this.watchPathExistence(p, watchPathExistenceCb);

        function watchPathExistenceCb(err, exists) {
                if (!run) {
                        return (false);
                }
                ...
                if (!exists) {
                        if (state !== states.NO_NODE &&
                                state !== states.BAD_DATA) {
                                state = states.NO_NODE;
                                return (callback(err));
                        }
                        return (false);
                }

                var params = {method: 'data', initialData: true};
                return (zk.watch(p, params, watchcb));
        }
        ...
        function watchcb(err, listener) {
                ...
                listener.on('error', errListener);
                listener.on('session', sessionListener);
                return (listener.on('data', dataListener));
        }
        ...
        function errListener(err) {
                ...
                if (state !== states.BAD_DATA && state !== states.NO_NODE) {
                        state = states.BAD_DATA;
                        return (callback(err));
                }
                return (false);
...
// transforms '/your/complex/path' to
// ['/', '/your', '/your/complex', '/your/complex/path']
function allPaths(p) {
        p = path.normalize(p);
        var parts = p.split('/');
        var paths = [];
