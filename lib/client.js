...
ZKClient.prototype.onclose = function () {
        console.log("onClose");
        var log = this.log;
        if (this.closed_by_client) {
                this.closed_by_client = undefined;
                log.info('The zookeeper session is closed by the client.');
                this.emit('close');
        }
        else {
                //the session has expired
                //It means that a disconnection has took more
                //that the timeout value.
                //So In this case, the servers has stopped the session and
                ...
                if (this.autoReconnect) {
                        ...
                        log.info('Auto reconnection');
                        this.reconnect();
                }
                else {
                        log.info('Let the application manage the session expiration');
...
//We can't reuse the same ZKClient because of
ZKClient.prototype.reconnect = function resetConnection() {
        this.zk = new ZK(this.zookeeperOptions);
...
ZKClient.prototype.create = ZKClient.prototype.create;
