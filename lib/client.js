...
ZKClient.prototype.pathExists = function(p, callback, recurse, run) {
    var parents;
    ...
    if (parents.length > 1) {
        ...
        zk.watch(cur, {method: 'list', initialData: true}, function(err, listener) {
            if (err !== null) {
                return callback(err);
            }

            listener.on('error', function(err) {
                if (!run) { return listener.stop(); }
                callback(err);
            });
            listener.on('children', function(children) {
                ...
                if (children.indexOf(path.basename(next)) !== -1) {
                    if (next === last) {
                        ...
                        zk.pathExists(parents.slice(1), callback, true, run);
                    }
                } else {
                    callback(err, false);
                }
...
ZKClient.prototype.uwatch = function uwatch(p, callback) {
    ...
    var pathrun = this.pathExists(p, function(err, exists) {
        if (!run) {
            return;
        }
        ...
        if (!exists) {
            if (state !== states.NO_NODE && state !== states.BAD_DATA) {
        ...
        zk.watch(p, {method: 'data', initialData: true}, function(err, listener) {
            if (err !== null) {
                return callback(err);
            }
    ...
    return {stop: function() {run = false; pathrun.stop()}};
}

// transforms '/your/complex/path' to ['/', '/your', '/your/complex', '/your/complex/path']
function allPaths(p) {
    p = path.normalize(p);
    var parts = p.split('/');
    var paths = [];
