...
function onClose() {
        var log = this.log;
        log.trace('zookeeper session closed by caller');
...
function onError(zh, _path, code) {
        var log = this.log;
        log.trace({
                zh: zh,
                path: _path,
        ...
        case ZK.ZCONNECTIONLOSS:
                this.emit('error', new ZKError(code, 'ZK: connection lost'));
                break;
...
function ZKClient(options) {
        ...
        this.zk.on('error', onError.bind(this));
        this.zk.on('session_expired', function () {
                self.log.trace('session expired');
                self.close();
        ...
        this.zk.once('close', onClose.bind(this));
        this.zk.on('connection_interrupted',
                   this.emit.bind(this, 'connection_interrupted'));
        this.zk.on('connect', this.emit.bind(this, 'connect'));
}
