...
function ZKClient(options) {
        ...
        assert.optionalNumber(options.timeout, 'options.connectTimeout');
        ...
        this.zk.on('session_expired', function () {
                ...
                if (self.listeners('session_expired') === 0) {
                        self.emit('error', ZK.ZSESSIONEXPIRED);
                }
                self.close();
        });
...
ZKClient.prototype.onError = function onError(zh, _path, code) {
        ...
        switch (code) {
                case ZK.ZCONNECTIONLOSS:
                        log.error('emitting connection_loss');
                        self.emit('connection_loss');
                        if (self.listeners('connection_loss') === 0) {
                                self.emit('error', ZK.ZCONNECTIONLOSS);
                        }
                        break;
